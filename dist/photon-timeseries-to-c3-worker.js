(function () {
  ...
  */
  ...
  var _looseJSON = {}; // LooseJSON.parse adapted from Canop's JSON,parseMore https://github.com/Canop/JSON.parseMore/
  ...
  var error = function error(m) {
    throw new Error("Syntax error - ".concat(m, " at char ").concat(at, " from '").concat(text, "'"));
  };
  ...
  _looseJSON.stringify = function (object) {
    ...
    var json = JSON.stringify(object, function (key, value) {
      ...
      return value;
    }).replace(/"__BigInt__([0-9.-eE]*)__"/g, '$1').replace(/"__NaN__"/g, 'NaN').replace(/"__Infinity__"/g, 'Infinity').replace(/"__-Infinity__"/g, '-Infinity');
    return json;
  };

  var looseJSON = _looseJSON;

  var _timeseriesTools = {};
  /**
  ...
  _timeseriesTools.isArray = function (value) {
  ...
  _timeseriesTools.isTimeseries = function (timeseries) {
    return !(!timeseries || timeseries === null || timeseries.c === null || timeseries.l === null || timeseries.a === null || timeseries.v === null || !_timeseriesTools.isArray(timeseries.v));
  };
  /**
  ...
  _timeseriesTools.jsonToStack = function (jsonList, prefixId) {
    ...
    jsonList.forEach(function (item, index) {
      ...
      if (_timeseriesTools.isArray(item)) {
        stack.push.apply(stack, _toConsumableArray(_timeseriesTools.jsonToStack(item, id)));
      }
  ...
  _timeseriesTools.sameMetadata = function (a, b) {
    ...
    for (var _p in b.l) {
      if (!a.l.hasOwnProperty(_p)) return false;
    }
  ...
  _timeseriesTools.isPlottable = function (timeseries) {
    ...
    for (var i in timeseries.v) {
      if (timeseries.v[i][timeseries.v[i].length - 1] !== null) {
        if (typeof timeseries.v[i][timeseries.v[i].length - 1] === 'number' || // if it's an instance of Big.js we get a `toFixed()` function in the prototype
  ...
  _timeseriesTools.isAnnotable = function (timeseries) {
    if (!_timeseriesTools.isTimeseries(timeseries) || timeseries.v.length === 0) {
      return false;
    } // We look at the first non-null value, if it's a String or Boolean it's an timeseries of annotations,
    // if it's a number it's a timeseries to plot
  ...
  _timeseriesTools.sort = function (timeseries) {
  ...
  _timeseriesTools.timeRange = function (timeseries) {
  ...
  _timeseriesTools.timeseriesToC3 = function (stack, options) {
    var inTimestamp = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : false;
    var data = {
      xs: {},
      types: {},
      names: {},
  ...
  var timeseriesTools = _timeseriesTools;
  ...
  onmessage = function onmessage(message) {
    ...
    var stack = timeseriesTools.jsonToStack(json);
    console.log('[Worker: timerseries-to-c3] stack', stack);
    var c3Data = timeseriesTools.timeseriesToC3(stack);
    console.log('[Worker: timerseries-to-c3] c3Data', c3Data);
    postMessage({
      c3: c3Data,
      __meta: message.data.__meta
    });
  };
